...
#### Full Pipeline (Extract + Evaluate)

1. **Extract shipment data from 50 sample emails:**
   ```bash
   python extract.py
   ```
   - Processes `emails_input.json`
   - Generates `output.json` with extracted data
   - Takes 5-10 minutes (subject to Groq API rate limits)

2. **Evaluate accuracy against ground truth:**
   ```bash
   python evaluate.py
   ```
   - Compares `output.json` with `ground_truth.json`
   - Displays per-field accuracy and overall accuracy
   - Example output:
     ```
     Per-field accuracy:
     - product_line: 95.92% (47/49)
     - origin_port_code: 93.88% (46/49)
     - destination_port_code: 95.92% (47/49)
     - incoterm: 97.96% (48/49)
     - cargo_cbm: 95.92% (47/49)
     - is_dangerous: 100.00% (49/49)
     Overall accuracy: 96.33% (423/439)
     ```
...
python evaluate.py
```
- Uses rule-based extraction instead of LLM
...
├── utils.py                   # Helper functions (port matching, parsing, etc.)
├── prompts.py                 # LLM prompts
├── emails_input.json          # 50 sample emails for testing
├── ground_truth.json          # Expected outputs (for evaluation)
├── port_codes_reference.json  # UN/LOCODE port mappings
...
After running `python extract.py`, you'll get `output.json` with entries like:
```json
{
  "id": "EMAIL_001",
...
| Issue | Solution |
|-------|----------|
| `GROQ_API_KEY not found` | Set API key in `.env` or environment variable |
| `ModuleNotFoundError: groq` | Run `pip install -r requirements.txt` |
| `Rate limit exceeded` | The script retries with exponential backoff (up to 3 attempts). Wait a moment and try again. |
| `Empty output.json` | Check logs for API errors. Try with `--mock` flag first. |
| `evaluate.py shows 0% accuracy` | Ensure `output.json` was generated successfully. Check file format is valid JSON. |
